// Description:
// Matching algorithm for general graphs (non-bipartite)
...
// vector<pii> Blossom(vector<vector<int>>& graph) {
vector<int> Blossom(vector<vector<int>>& graph) {
  ...
  vector<pii> ans;
  for (int i = 0; i < n; i++) {
    if (matching[i] == -1 || used[i]) continue;
    used[i] = true;
    used[matching[i]] = true;
    ans.emplace_back(i, matching[i]);
  }
  return ans;
  */
}
